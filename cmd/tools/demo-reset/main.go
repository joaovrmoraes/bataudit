...
const resetHourUTC = 3 // 00:00 BRT = 03:00 UTC
...
func main() {
	...
	for {
		next := nextResetTime()
		slog.Info("next demo reset scheduled", "at", next.Format(time.RFC3339))
		time.Sleep(time.Until(next))

		slog.Info("starting demo reset")
		if err := resetAndReseed(conn); err != nil {
...
func nextResetTime() time.Time {
	now := time.Now().UTC()
	next := time.Date(now.Year(), now.Month(), now.Day(), resetHourUTC, 0, 0, 0, time.UTC)
	if !next.After(now) {
